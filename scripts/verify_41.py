...
def main():
    ...
    check("AgingCell memakai keadaan SLA dari server", "state" in src and "sla_state" in src)
    ...
    if rows:
        ...
        first = rows[0]
        due_ok = bool(first.get("stage_due_at")) and first["stage_due_at"] > first["stage_entered_at"]
