...
export default function PartnersListTab() {
  ...
  const { user } = useAuth();
  const canManage = ["owner", "super_admin", "sales_manager", "marketing_admin",
    "dm_supervisor"].includes(user?.role);
  ...
  return (
    <div className="space-y-3">
      ...
      <DataTable testId={PARTNERS.table}
        ...
        emptyActionLabel={activeCount || query.q ? "Reset filter" : (canManage ? "Tambah Mitra" : "")}
        emptyAction={activeCount || query.q ? () => reset()
          : (canManage ? () => setFormFor({}) : null)} />
